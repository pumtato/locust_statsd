...
class StatsdClient(object):
    ...
    def timing(self, stats, value):
        ...
        """
        self._update_stats(stats, value, self.SC_TIMING)
    ...
    def gauge(self, stats, value):
        ...
        """
        self._update_stats(stats, value, self.SC_GAUGE)
    ...
    def set(self, stats, value):
        ...
        """
        self._update_stats(stats, value, self.SC_SET)
    ...
    def increment(self, stats, sample_rate=1):
        ...
        """
        self._count(stats, 1, sample_rate)
    ...
    def count(self, stats, value, sample_rate=1):
        ...
        """
        self._count(stats, value, sample_rate)

    def _count(self, stats, value, sample_rate=1):
        self._update_stats(stats, value, self.SC_COUNT, sample_rate)

    def _update_stats(self, stats, value, _type, sample_rate=1):
        """
        Pipeline function that formats data, samples it and passes to _send()

        >>> client = StatsdClient()
        >>> client.update_stats('example.update_stats', 73, "c", 0.9)
        """
        stats = self._format(stats, value, _type)
        self._send(self._sample(stats, sample_rate))

    @staticmethod
    def _format(keys, value, _type):
        """
        General format function.

        >>> StatsdClient._format("example.format", 2, "T")
        {'example.format': '2|T'}
    ...
    def _send(self, _dict):
        """
        Sends key/value pairs via UDP.

        >>> StatsdClient._send({"example.send":"11|c"}, ("127.0.0.1", 8125))
        """
        for item in _dict.items():
            self._udp_sock.sendto(":".join(item).encode('utf-8'), self._addr)


if __name__ == "__main__":
    client = StatsdClient()
    client.increment("test.met")
